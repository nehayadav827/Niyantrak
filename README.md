...
Instead of requiring a known corridor name, Niyantrak resolves location intelligence directly from coordinates, then layers a two-stage ML forecast with rule-based operational logic on top:
...
└────────────────────┬────────────────────────┘
                     ...
                     ▼
┌───────────────────────────────────────────┐
│  Zero-Inflated CatBoost Hurdle Model       │  Classifier estimates alert
│  (classifier + regressor + quantiles)      │  probability; regressor estimates
│                                            │  count; quantile models bound it.
└────────────────────┬────────────────────────┘
...
> **Note:** the live repository's top level also contains `traffic_web/`, `main.py`, `risk_main.py`, `risk_main_v2.py`, and `check.py`, which are not yet reconciled with the structure above — see [Architecture](docs/architecture.md#project-structure) for the open discrepancy.
...
| Forecasting model | CatBoost (Classifier + Regressor + Quantile Regressors) | Zero-inflated hurdle model + 80% prediction interval |
